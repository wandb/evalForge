...
class PythonAssertion(BaseModel):
    test_name: str = Field(..., description="A clear, concise name for the unittest.TestCase method with function name in snake case and starting with test_. Must be identical to the function name in the code field.")
    code: str = Field(..., description="A clear, concise assertion written as a unittest.TestCase method with function name in snake case and starting with test_. Also ensure that the input to the function is only self and the function accesses self.output.")
    evaluation_type: Literal["python"] = "python"

class LLMAssertion(BaseModel):
    test_name: str = Field(..., description="A clear, concise name for the LLM evaluator in snake case.")
    text: str = Field(..., description="A detailed prompt for the LLM to evaluate the assertion, guiding it to return 'PASS' or 'FAIL'")
